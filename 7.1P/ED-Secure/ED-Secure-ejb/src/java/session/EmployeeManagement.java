...
public class EmployeeManagement implements EmployeeManagementRemote {
    ...
    private Employee employeeDTO2EntityPartial(EmployeeDTO empDTO) {
        if (empDTO == null) {
            // just in case
            return null;
        }
        ...
        if (appGroup != null) employee.setAppGroup(appGroup);
        if (bankAccountId != null) employee.setBnkAccId(bankAccountId);
